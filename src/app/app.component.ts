...
export class AppComponent {
  ...
  filteredList : Array<IbookDetails> = [] ;
  cartTotal : number = 0 ; 
  private _listFilter = "" ;
  get listFilter(){
  ...
  set listFilter(value : string){
    ...
    this.filteredList = this.filterListitems(value);
  }
  addToCartList(book : IbookDetails ) : void {
    const temp = this.cartList.findIndex((item) => item.ISBN == book.ISBN) ;
    if (temp === -1 ) {
      book.qty! = 1 ;
      this.cartList.push(book) ;
    }
    else {
      this.cartList[temp].qty = this.cartList[temp].qty! + 1 ; 
    }
    this.cartTotal += book.price.value ;
  }

  removeFromCartChild(boook : IbookDetails) : void {
    const index = this.cartList.findIndex((item) => item.ISBN == boook.ISBN) ;
    if (index != -1 ){
      const check = this.cartList[index].qty ;
      this.cartTotal -= this.cartList[index].price.value ;
      if (check == 1){
  ...
  quantityChange( isbn : number, str : string ) : void { 
    const index = this.cartList.findIndex((item) => item.ISBN === isbn) ;
    if (index != -1 ) {
      if (str === '+') {
        this.cartList[index].qty = this.cartList[index].qty! + 1 ;
        this.cartTotal += this.cartList[index].price.value ;
  ...
  removeItemFromCart(isbn : number) :void {
    const temp = this.cartList.findIndex((item) => item.ISBN === isbn) ;
    if (temp != -1 ) {
      this.cartTotal -= (this.cartList[temp].price.value * this.cartList[temp].qty!) ;
      this.cartList[temp].qty = 0 ;
      this.cartList.splice(temp , 1 ) ;
    }
  }

  filterListitems(filter : string ){
    filter = filter.toLowerCase() ;
    return this.bookList.filter((item) => item.title.toLowerCase().includes(filter)) ;
  }

  filterBookListHigh() {
    const temp = [...this.filteredList].sort(function (a : IbookDetails, b : IbookDetails){return a.price.value - b.price.value}) ;
    this.filteredList = temp ;
  }
  filterBookListLow() {
    const temp = [...this.filteredList].sort(function (a : IbookDetails, b : IbookDetails){return b.price.value - a.price.value}) ;
    this.filteredList = temp ;
  }
